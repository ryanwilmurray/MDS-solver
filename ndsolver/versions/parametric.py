...
class MM:
    ...
    def __init__(self, X: np.ndarray, labels: list, m=2, init: np.ndarray = None, init_scaling_factor=1.25):
        ...
        self.a, self.b, self.c = self.initialize_parameters()
    ...
    def initialize_parameters(self):
        ...
        return a.flatten(), b, c
    
    
    def grad(self) -> np.ndarray:
        Y = parametric_root_finder_1d(self.X, self.a, self.b, self.c)
        X_norms_squared = LA.norm(self.X, axis=0)**2
        M2 = np.mean(X_norms_squared)
        
        return np.sum(
            ((LA.norm(self.X[:,i] - self.X[:,j])**2 - (Y[i] - Y[j])**2) * (Y[i] - Y[j])) * np.array([*self.X[:,i], -Y[i], -1]) /
            (3 * Y[i]**2 - LA.norm(self.X[:,i])**2 - M2 - self.b)
    ...
    def update_state(self, step_size = 100, iterations=1000):
        ...
        self.Y = Y-np.mean(Y, axis=1, keepdims=True)
        #self. Y /= np.std(self.Y,axis=1, keepdims = True)
        self.psi, self.phi = self.initialize_parameters() 
        self.cost = self.get_cost()
